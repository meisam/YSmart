...
package edu.osu.cse.ysmart.tpch3query;
...
public class tpch3query002 extends Configured implements Tool{
	...
	public static class Map extends  Mapper<Object, Text,IntWritable,Text>{
		...
		public void setup(Context context) throws IOException, InterruptedException {
			...
			if(filename.compareTo("tpch3query003")==0){
				filetag = 1;
			}
		...
		public void map(Object key, Text value,Context context) throws IOException,InterruptedException{
			...
			if(filetag ==1){
				...
				context.write(new IntWritable(Integer.parseInt(line_buf[2])),new Text(1+"||"+line_buf[0]+ "|" +Integer.parseInt(line_buf[1])+ "|" +Integer.parseInt(line_buf[2])+ "|" ));
			}

			if(filetag ==2){

				if(line_buf[10].compareTo("1995-03-26") > 0){

					if(!(line_buf[10].compareTo("1995-03-26") > 0))
						dispatch.set( 0 );
					if(dispatch.isEmpty())
						context.write(new IntWritable(Integer.parseInt(line_buf[0])),new Text(2+"||"+Integer.parseInt(line_buf[0])+ "|" +Double.parseDouble(line_buf[5])+ "|" +Double.parseDouble(line_buf[6])+ "|" ));
					else
						context.write(new IntWritable(Integer.parseInt(line_buf[0])),new Text(2+"|"+dispatch.toString()+"|"+Integer.parseInt(line_buf[0])+ "|" +Double.parseDouble(line_buf[5])+ "|" +Double.parseDouble(line_buf[6])+ "|" ));
				}
			}
		...
		public void reduce(IntWritable key, Iterable<Text> v, Context context) throws IOException,InterruptedException{
			...
			for(int i=0;i<al_left_0.size();i++){
				...
				for(int j=0;j<al_right_0.size();j++){
					...
					it_output[0].add(Integer.parseInt(right_buf_0[0])+ "|" +left_buf_0[0]+ "|" +Integer.parseInt(left_buf_0[1])+ "|" +Double.parseDouble(right_buf_0[1])+ "|" +Double.parseDouble(right_buf_0[2])+ "|" );
				}
			}
			...
			for(int i=0;i<it_output[0].size();i++){
				...
				if(jfc_gb_output_0[0].containsKey(tmp_buf[0]+"|"+tmp_buf[1]+"|"+tmp_buf[2]+"|")){
					Double sum_tmp = (double)((Double.parseDouble(tmp_buf[3]) * (1 - Double.parseDouble(tmp_buf[4]))));
					 sum_tmp += jfc_gb_output_0[0].get(tmp_buf[0]+"|"+tmp_buf[1]+"|"+tmp_buf[2]+"|");
					jfc_gb_output_0[0].put(tmp_buf[0]+"|"+tmp_buf[1]+"|"+tmp_buf[2]+"|", sum_tmp);
				}else{
					jfc_gb_output_0[0].put(tmp_buf[0]+"|"+tmp_buf[1]+"|"+tmp_buf[2]+"|",(double)((Double.parseDouble(tmp_buf[3]) * (1 - Double.parseDouble(tmp_buf[4])))));
				}
			}
			for(Object tmp_key:jfc_gb_output_0[0].keySet()){
				...
				for(int i=0;i<it_output[0].size();i++){
					...
					if(((String)tmp_key).compareTo(tmp_buf[0]+"|"+tmp_buf[1]+"|"+tmp_buf[2]+"|")==0)
						break;
				}
				String tmp_result = "";
	...
	public int run(String[] args) throws Exception{
		...
		Job job = new Job(conf,"tpch3query002");
		job.setJarByClass(tpch3query002.class);
	...
	public static void main(String[] args) throws Exception {
			...
			int res = ToolRunner.run(new Configuration(), new tpch3query002(), args);
			System.exit(res);
	}
