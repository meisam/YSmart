...
public class tpch18query003 extends Configured implements Tool{
		...
		public void map(Object key, Text value,Context context) throws IOException,InterruptedException{
			...
			if(filetag ==1){
				...
				if(true || true){
					...
					if(dispatch.isEmpty())
						context.write(new IntWritable(Integer.parseInt(line_buf[0])),new Text(1+"||"+Double.parseDouble(line_buf[4])+ "|" +Integer.parseInt(line_buf[0])+ "|" ));
					else
						context.write(new IntWritable(Integer.parseInt(line_buf[0])),new Text(1+"|"+dispatch.toString()+"|"+Double.parseDouble(line_buf[4])+ "|" +Integer.parseInt(line_buf[0])+ "|" ));
				}
			}
			...
			if(filetag ==2){
				...
				if(true){
					...
					if(dispatch.isEmpty())
						context.write(new IntWritable(Integer.parseInt(line_buf[0])),new Text(2+"||"+Integer.parseInt(line_buf[1])+ "|" +Integer.parseInt(line_buf[0])+ "|" +line_buf[4]+ "|" +Double.parseDouble(line_buf[3])+ "|" ));
					else
						context.write(new IntWritable(Integer.parseInt(line_buf[0])),new Text(2+"|"+dispatch.toString()+"|"+Integer.parseInt(line_buf[1])+ "|" +Integer.parseInt(line_buf[0])+ "|" +line_buf[4]+ "|" +Double.parseDouble(line_buf[3])+ "|" ));
				}
			}
		...
		public void reduce(IntWritable key, Iterable<Text> v, Context context) throws IOException,InterruptedException{
			...
			while(values.hasNext()){
				...
				if(line.charAt(0) =='1'&& (dispatch.length()==0 ||dispatch.indexOf('1')==-1)){
					result_1[0] +=Double.parseDouble(line_buf[0]);
					al_line_1++;
				}
			}
			String[] line_buf = tmp.split("\\|");
			for(int i=0;i<al_left_0.size();i++){
				...
				for(int j=0;j<al_right_0.size();j++){
					...
					if(Integer.parseInt(right_buf_0[1]) == Integer.parseInt(left_buf_0[1])){
						it_output[0].add(Integer.parseInt(left_buf_0[0])+ "|" +Integer.parseInt(left_buf_0[1])+ "|" +left_buf_0[2]+ "|" +Double.parseDouble(left_buf_0[3])+ "|" +Double.parseDouble(right_buf_0[0])+ "|" );
					}
				}
			}
			if(result_1[0] > 300)
				it_output[1].add(Integer.parseInt(line_buf[1]) + "|"+(result_1[0]) + "|");
			...
			for(int i=0;i<it_output[0].size();i++){
				...
				for(int j=0;j<it_output[1].size();j++){
					...
					jfc_output[0].add(Integer.parseInt(jfc_left_buf_0[0])+ "|" +Integer.parseInt(jfc_left_buf_0[1])+ "|" +jfc_left_buf_0[2]+ "|" +Double.parseDouble(jfc_left_buf_0[3])+ "|" +Double.parseDouble(jfc_left_buf_0[4])+ "|" );
				}
			}
	...
	public int run(String[] args) throws Exception{
		...
		Job job = new Job(conf,"tpch18query003");
		job.setJarByClass(tpch18query003.class);
	...
	public static void main(String[] args) throws Exception {
			...
			int res = ToolRunner.run(new Configuration(), new tpch18query003(), args);
			System.exit(res);
	}
